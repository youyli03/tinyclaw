...
- 🔑 **权限跟着目标会话**（2026-09-25 修正）：job 是用户在那个会话里起出来办事的，所以"干完活叫醒我"
  不该被降级。目标会话有可交互通道（qqbot）时，这一轮按该会话的**普通对话权限**跑 —— 全量工具，
  需要 MFA 时提示发到那个 QQ 聊天等你回复（`origin=wake`，审计里可追溯 `source`）；
  目标没有交互路径（`cli:` / 无常驻连接）时退回**无人值守**（`origin=cron`，白名单 + MFA 一律拒绝）。
  也就是说：`job_*` 这类工具在 QQ 唤醒的那一轮里是可用的（在 cli 目标下不可用）；
