...
registerCommand({
  ...
  execute({ session }) {
    ...
    if (session.mode === "code") {
      return "i️ 已处于 Code 模式。发送 `/chat` 可返回聊天模式。";
    }
    session.mode = "code";
    ...
    // 如果有项目绑定,尝试重新获取锁并恢复 project 工作目录
    let lockMsg = "";
    if (session.projectSlug && session.sessionId) {
      const reacquired = acquireLock(session.agentId, session.projectSlug, session.sessionId);
      if (!reacquired) {
        lockMsg = "\n• ⚠️ 项目锁已被其他 session 持有(项目上下文仍可用)";
      }
      // 从 metadata.json 恢复 workdir(优先于 .code.dir 的值)
      try {
        ...
        if (fs.existsSync(metaPath)) {
          ...
          if (meta?.project?.workdir) {
            session.codeWorkdir = meta.project.workdir;
          }
        }
      } catch {
        /* ignore */
    ...
    const dirLine = session.codeWorkdir
      ? `• 📁 工作目录:\`${session.codeWorkdir}\`${lockMsg}`
      : `• 📁 工作目录:\`${session.codeWorkdir ?? defaultDir}\`(默认,发送 \`/workspace <路径>\` 指定项目目录)`;
...
registerCommand({
  ...
  execute({ session }) {
    ...
    if (session.mode === "chat") {
      return "i️ 已处于聊天模式。发送 `/code` 可切换到 Code 模式。";
    }
    session.mode = "chat";

    // 离开 code 模式时释放项目锁(保留 projectSlug 以便 /code 自动恢复)
    if (session.projectSlug && session.sessionId) {
      releaseLock(session.agentId, session.projectSlug, session.sessionId);
    }
...
registerCommand({
  ...
  execute({ session }) {
    ...
    if (session.codeSubMode === "plan") {
      return "i️ 已处于 Plan 子模式。发送 `/auto` 可切换回直接执行模式。";
    }
