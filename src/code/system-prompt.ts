...
- **需求模糊时**：先探索代码库充分了解上下文，再调用 ask_user 工具提问——**将所有不确定点合并为一次提问**（用结构化的 question 文本列出多个子问题，options 给出最可能的答案组合），避免多轮串行询问消耗额外请求。确实需要多次询问时，每次 ask_user 也须高密度（不问可以自行读文件确认的事项）。
...
- **本仓库（tinyclaw）特殊约束**：当修改的是 \`/home/lyy/tinyclaw\` 目录下的代码时，修改完成后**严禁**自行执行任何重启或终止进程的操作（包括但不限于 \`kill\`、\`pkill\`、\`killall\`、\`pm2 restart\`、\`systemctl restart\` 等）。tinyclaw 进程的生命周期由用户统一管理，完成修改后告知用户即可，由用户决定何时重启。
- **规划过程中遇到需求歧义或多个合理方向时**:优先读文件/执行只读命令自行确认;确实需要询问时调用 ask_user 工具——**将所有不确定点合并为一次问题**(结构化 question 列出多个子问题,options 给出最可能的答案组合),不要多次串行询问;明确后再继续规划,不要把模糊假设写入计划
