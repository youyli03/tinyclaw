...
registerTool({
  ...
  execute: async (_args: Record<string, unknown>, ctx?: ToolContext): Promise<string> => {
    ...
    if (!fs.existsSync(memPath)) {
      return "（MEM.md 尚不存在，可调用 memory_write_mem 创建）";
    }
    const content = fs.readFileSync(memPath, "utf-8");
    return content || "（MEM.md 为空）";
  },
});
...
registerTool({
  ...
  execute: async (args: Record<string, unknown>, ctx?: ToolContext): Promise<string> => {
    ...
    if (mode === "append") {
      fs.appendFileSync(memPath, content, "utf-8");
      return `已追加到 MEM.md（${content.length} 字节）：${memPath}`;
    } else {
      fs.writeFileSync(memPath, content, "utf-8");
      return `已覆写 MEM.md（${content.length} 字节）：${memPath}`;
    }
  },
});
