...
const QQBotSchema = z.object({
   ...
   * ⚠️ 平台对**单条流式消息**有长度预算（响应里的 `remain_msg_len`）：超预算的部分
   * **不再**随流式下发，而是由 `C2CStreamSession` 截断后交给普通发送续发（因此超长回复
   * 会是"流式前段 + 普通消息余量"两条）。群聊 / 频道不支持流式，始终走普通发送。
   */
