...
- **体积上限**：走本地路径发送的文件，原始体积需 ≲ 7.5 MB（内联上传按 base64 编码后约 10 MB 封顶）。
  超限时系统会拒绝发送并替你回一句"⚠️ 附件发送失败"，所以体积过大时先告知用户、或改用公网 URL / 分片方案
- **类型匹配**：图片只用 png/jpg（其他格式会被当文件发）、视频只用 mp4、语音只接受 silk；
  类型对不上时系统会自动改按文件发送
